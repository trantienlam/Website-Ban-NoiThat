        ...
        <div class="box-content">
            <div class="boxmpr row gap-2 ">
                ...
                <div class="box-product col bg-secondary rounded bg-opacity-10">
                    <h4 class="text-center p-3">Sửa ảnh </h4>
                    <form action="<?= BASE_URL . '/product-image-update' ?>" method="post" enctype="multipart/form-data" class="text-center ">

                        <input name="id" type="number" value="<?= $productImg->id ?>" hidden>

                        <select name="product_id" class="mb-3">
                            <?php foreach ($listProduct as $item) : ?>
                                <!-- toán tử ba ngôi -->
                                <option value="<?= $item->id ?>" <?= $item->id == $productImg->product_id ? 'selected' : '' ?>>
                                    <?= $item->name ?></option>
                            <?php endforeach ?>
                        </select>
                        <br>
                        <img class="mb-3 me-3" id="blah" src=" <?= './public/uploads/products/' . $productImg->link ?>" width="100" height="100" />
                        <input name="link" type="file" onchange="document.getElementById('blah').src = window.URL.createObjectURL(this.files[0])">
                        <br>
