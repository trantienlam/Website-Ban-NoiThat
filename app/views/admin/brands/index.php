                ...
                <div class="box-product col bg-secondary rounded bg-opacity-10">
                    <div class="bg-white my-3 rounded">
                        <div class="d-flex justify-content-between align-items-center p-2">
                            <h5 class="fw-bold">List Brands</h5>
                            <form class="d-flex align-items-center" method="POST" action="<?= BASE_URL . '/brands-show' ?>" autocomplete="off">
                                <div class="col-auto">
                                    <input type="text" class="form-control" placeholder="Tên thương hiệu theo name" name="tim-kiem-thuong-hieu" id="tim-kiem-thuong-hieu" required>
                                </div>
                                <div class="col-auto">
                                    <button type="submit" class="btn btn-success ms-2">Tìm kiếm</button>
                                </div>
                            </form>
                            <a href="<?= BASE_URL . '/brands-create' ?> " class="btn btn-primary mb-2 px-3">Thêm Thương Hiệu</a>
                        </div>
                        <div class="p-2">
                            <table class="table">
                                ...
                                <tbody>
                                    <?php foreach ($listBrands as $key => $item) : ?>
                                        <tr>
                                            ...
                                            <td><?= $item->name ?></td>
                                            <td class="d-flex p-3 ">
                                                <form action="<?= BASE_URL . '/brands-edit' ?>" class="me-2">
                                                    <input type="text" name="id" value="<?= $item->id ?>" hidden>

                                                    <button class="btn text-white bg-info"><iconify-icon icon="ph:note-pencil-bold"></iconify-icon></button>
                                                </form>
                                                <form action="<?= BASE_URL . '/brands-destroy' ?>" class="">
                                                    <input type="text" name="id" value="<?= $item->id ?>" hidden>

                                                    <button class=" btn text-white bg-danger" onclick="return confirm('Bạn có chắc chắn muốn xóa thương hiệu không')"><iconify-icon icon="material-symbols:delete-rounded"></iconify-icon></button>
                                                </form>
                                            </td>
                                        </tr>
                                    <?php endforeach ?>
